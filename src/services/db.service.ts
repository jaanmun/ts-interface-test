...
export const leftJoinDocument =
  (firestore: Firestore, field, collection) => (source) =>
    defer(() => {
      // Operator state
      let collectionData;
      const cache = new Map();
...
export const docJoin =
  (firestore: Firestore, field: string, collection: string) => (source) =>
    defer(() => {
      let dData;
      return source.pipe(
        switchMap((data) => {
        ...
        map((fieldData) =>
          fieldData ? { ...dData, [field]: fieldData } : dData
        )
      );
    });
...
export const ArrayleftJoinDocument =
  (firestore: Firestore, field, collectionName) => (source) =>
    defer(() => {
      // Operator state
      let colData;
      const cache = new Map();
...
export class DbService {
  ...
  async toCollection$(collectionName, queryFn?) {
    const result = [];
    if (queryFn) {
      const queryData = this.parseQuery(queryFn);
  ...
  async toDoc$(path) {
    ...
    return (await getDoc(doc(this.firestore, pathArr[0], pathArr[1]))).data();
  }
